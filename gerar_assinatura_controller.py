...
def assinar_documentos_pdfa(documento_pdfa):
    ...
    return criar_pdf_com_assinatura(documento_pdfa, assinatura, qrcode)


def criar_pdf_com_assinatura(documento_pdfa, assinatura, qr_code):
    doc = fitz.open(documento_pdfa)

    # Convert hash to hex string for readability
    hash_text = assinatura.hex()
